...
- select number N such that selecting only nodes with trailing zeroes >= N totals >= 2^N nodes and selecting only nodes with log2 absolute difference <= (L-N-1) totals >= 2^N nodes aswell
